...
export function TableHead({ locale }: ITableHeadProps) {
    const { header } = strings[locale].table; // TODO #2 opportunity: use context instead of props (low value)
    ...
    return (
        <thead class="table-head">
            <tr class="table-head-first-row">
                ...
                <th colSpan={4} class="matches">{header.matches.name}</th>
                <th colSpan={2} class="goals">{header.goals.name}</th>
